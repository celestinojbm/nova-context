...
| M17C | Acquisition Readiness Baseline v0 (audit/documentation milestone — NO product features, NO deploy, NO data room, NO private records, NO legal conclusions): permanent strategic lens "build Nova as a transferable, acquirable technology asset". `docs/acquisition/`: README (boundaries, no-secrets/no-legal-conclusion rules, cadence), ACQUISITION_READINESS (verified complete/partial/absent/not-verified/premature classification; top-5 risks; 5 highest-impact actions; single next action = chain-of-title verification), ACQUISITION_SCORE.json (schema v1, baseline `74ac864c`, **59/100**, recalculated from repo evidence; caps documented: IP-unverified ≤70, no-real-deploy → infra ≤6/12, no-market-evidence limits product/business; NOT a valuation), EVIDENCE_INDEX (EV-01…16 → existing artifacts, proves/does-not-prove, freshness), RISK_REGISTER (R-01…12; P0 = IP chain of title unverified — no LICENSE file, AI-assisted authorship undocumented), IP_AND_TRANSFERABILITY_CHECKLIST (repo-verifiable vs private/off-repo; ownership NOT VERIFIED — never assert personal vs Stravos Enterprises LLC without documents), DEPENDENCY_AND_LICENSE_INVENTORY (212 prod pkgs via pnpm licenses; 1 transitive LGPL prebuilt sharp-libvips, CC-BY caniuse-lite; no GPL/AGPL; NOTICE absent; professional review flagged), VALUE_DELTA_POLICY (none/brief/full cadence; baseline entry: comparison confidence n/a, no score-increase claim), DUE_DILIGENCE_READINESS (17-section living report). Consumes Validation Gate evidence, duplicates nothing. Real alpha STILL BLOCKED | this branch |
| M17B | Validation Gate v0 (thin orchestration layer — NOT the full Validation Harness v1, NOT a new test framework, NO deploy/provisioning/real user data): `tools/validation-gate/` (TypeScript, dependency-light) orchestrates the EXISTING aggregates with consistent PASS/CONDITIONAL_PASS/FAIL/BLOCKED semantics. Four modes: `validate:pr` (build→typecheck→test→db:migrate→test:integration; CI now invokes this instead of the raw steps — no duplication), `validate:predeploy` (operator prereqs by NAME + config safety [open-signup/redaction-off/unsafe-override/same-keys → FAIL] + feature posture + ops:preflight), `validate:postdeploy --base-url` (readyz ready:true + optional authed status leak-check + synthetic ops:smoke), `validate:recovery --backup-dir --stamp` (restore-guard scratch classification + backup:verify + EXPECTED wrong-key failure + guarded restore + migrate no-op + media:verify). Missing operator infra → honest BLOCKED (never PASS; names only, never values); exit PASS/COND→0, FAIL→1, BLOCKED→1(pr)/2(operator). Reports per run: JSON schema_v1 + Markdown verdict + JUnit (one testcase/check) under gitignored artifacts/validation/, uploaded in CI if:always() + job summary. Central sanitizer (env-value + pattern layers: DSNs, bearer/session/cookies, API-key shapes, 64-hex keys, any-case data: URLs, private keys) with 29 gate tests incl. secret-leak prevention, timeout kill, expected-failure both ways, cascade skips. Metrics observed-only (no p95/p99 SLAs — deferred with load/chaos/soak/trend-history to Harness v1 after first real synthetic deploy). **M17B.1 semantics hardening** (architecture review findings, all P1/P2 fixed): (1) recovery PASS now REQUIRES post-restore smoke — `--restored-base-url` is a hard prerequisite (loopback or explicit `NOVA_VALIDATE_ALLOW_REMOTE_RESTORED=yes`), `post_restore_smoke` required P1 category recovery (protected); (2) PURE config checks (config-safety/posture/prereqs) always run even after a required block — unsafe supplied config (open signup/redaction off/unsafe override/same keys) → FAIL even with missing infra (FAIL > BLOCKED), preflight never runs on missing prereqs; (3) authed `/v1/ops/status` mandatory for postdeploy PASS — `NOVA_VALIDATE_SESSION_TOKEN` hard prerequisite, check required P1 privacy, 3-layer leak detection (JSON contract + forbidden raw-error/content patterns + sanitizer diff); (4) structured skip provenance — `skip_reason`/`caused_by_check_id` typed fields; required skips valid ONLY as cascades from an earlier failed/blocked required check, never free-text inference. 53 gate tests. docs/VALIDATION_GATE.md | this branch |
